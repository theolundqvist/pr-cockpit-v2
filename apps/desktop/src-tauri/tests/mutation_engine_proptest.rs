...
fn payload_for_kind(kind: MutationKind, suffix: &str) -> SubmitPayload {
    ...
    let input_json = match kind {
        ...
        MutationKind::DeleteHeadRef => merge_with_base(serde_json::json!({})),
        MutationKind::EnqueueMergeQueue => merge_with_base(serde_json::json!({
            "expected_head_oid": "head",
            "jump": false,
        })),
        MutationKind::DequeueMergeQueue => merge_with_base(serde_json::json!({
            "merge_queue_entry_id": "queue-entry-1",
        })),
        MutationKind::ReorderMergeQueue => merge_with_base(serde_json::json!({
            "merge_queue_entry_id": "queue-entry-1",
            "mode": "TOP",
        })),
