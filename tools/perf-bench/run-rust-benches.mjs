...
async function main() {
  ...
  const mutationSubmitVisibleOnlineNs = await readEstimate("target/criterion/mutation_submit_visible_online/new/estimates.json");
  const mutationSubmitVisibleOfflineNs = await readEstimate("target/criterion/mutation_submit_visible_offline/new/estimates.json");
  ...
  const metrics = {
    ...
    mutation_submit_visible_online_ms: mutationSubmitVisibleOnlineNs / 1e6,
    mutation_submit_visible_offline_ms: mutationSubmitVisibleOfflineNs / 1e6,
    mutation_submit_visible_ms: Math.max(
      mutationSubmitVisibleOnlineNs,
      mutationSubmitVisibleOfflineNs,
    ) / 1e6,
