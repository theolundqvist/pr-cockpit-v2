...
| --- | --- | --- | --- | --- |
| Inbox first paint (warm cache) | `< 100 ms` | `92 ms` (`inbox_first_paint_ms_frontend`) | green | Frontend WebKit run; best-of-5 sample policy. |
...
| PR detail open (preloaded) | `< 50 ms` | `22 ms` (`pr_detail_open_preloaded_ms_frontend`) | green | Also `0.64 ms` in Rust microbench. |
| PR detail open (cold cache hit) | `< 250 ms` | `34 ms` (`pr_detail_open_cold_ms_frontend`) | green | Also `32.66 ms` in Rust microbench. |
| File open in diff (cached) | `< 100 ms` | `49 ms` (`file_open_in_diff_cached_ms_frontend`) | green | Frontend path remains well under hard budget and baseline tolerance. |
| File open with highlighting (viewport) | `< 300 ms` | `49 ms` (`file_open_in_diff_cached_ms_frontend`) | green | Highlight worker warm path stays under budget. |
| Comment submit visible | `< 16 ms` | `0.40 ms` (`mutation_submit_visible_ms`) | green | Worst-case of online/offline mutation microbench paths. |
...
| Diff scroll FPS | `>= 60 fps` | `62.15 fps` (`diff_scroll_fps`) | green | Also `16.2 ms` frame p95 (`diff_scroll_frame_p95_ms`). |
| `comrak_render_throughput_ops_per_sec` | `>= 850 ops/s` | `36491.60 ops/s` | green | Criterion throughput measure; no regression signal. |
| `mutation_submit_visible_ms` (explicit) | `<= 16 ms` | `0.40 ms` | green | Tracks PLAN §10 “comment submit visible” contract directly. |
...
- `inbox_first_paint_ms` (rust): `29.83 -> 29.00` (improved).
- `inbox_first_paint_ms_frontend`: `96 -> 92` (improved).
- `pr_detail_open_preloaded_ms_frontend`: `24 -> 22` (improved).
- `pr_detail_open_cold_ms_frontend`: `43 -> 34` (improved).
- `file_open_in_diff_cached_ms_frontend`: `73 -> 49` (improved).
- `diff_scroll_fps`: `62.20 -> 62.15` (stable).
- `command_palette_open_ms`: `2 -> 2` (stable).
- `command_palette_result_ms`: `0 -> 0` (stable).
- `comrak_render_throughput_ops_per_sec`: `36301.18 -> 36491.60` (stable, far above budget).
